...
func TolerationsTolerateTaint(logger klog.Logger, tolerations []v1.Toleration, taint *v1.Taint, enableComparisonOperators, enableSemverComparisonOperators bool) bool {
	for i := range tolerations {
		if tolerations[i].ToleratesTaint(logger, taint, enableComparisonOperators, enableSemverComparisonOperators) {
			return true
		}
	}
	return false
...
func FindMatchingUntoleratedTaint(logger klog.Logger, taints []v1.Taint, tolerations []v1.Toleration, inclusionFilter taintsFilterFunc, enableComparisonOperators, enableSemverComparisonOperators bool) (v1.Taint, bool) {
	filteredTaints := getFilteredTaints(taints, inclusionFilter)
	for _, taint := range filteredTaints {
		if !TolerationsTolerateTaint(logger, tolerations, &taint, enableComparisonOperators, enableSemverComparisonOperators) {
			return taint, true
		}
	}
